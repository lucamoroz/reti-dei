...
void crea_ip(struct ip_datagram *ip, int payloadsize,unsigned char proto, unsigned char ttl, unsigned char *ipdest) {
  ip-> ver_ihl = 0x45;
  ip->tos=0;
...
int main(){
  ...
  char flag_recvd_time_exeeded = 0;
  while (!flag_recvd_time_exeeded && ttl > -1) {

    crea_icmp_echo(icmp, 0x1234, 1);
    // 28: ICMP head + ICMP payload
    crea_ip(ip, 28, 1, ttl, iptarget);
    crea_eth(eth,mac,0x0800);

    n = sendto(s, buffer, 14 + 20 + 8 + 20, 0, (struct sockaddr*)&sll, sizeof(sll));
    if (n==-1) { perror("sendto fallita"); return 1; }

    // loop until time exceeded or echo response message
    while (1) {
      n = recvfrom(s, buffer, 1500, 0, (struct sockaddr*)&sll, &lungh);
      if (n==-1) { perror("recvfrom fallita"); return 1; }
      ...
      if (eth->type == htons(0x0800)
        ...
        printf("%s\n", "\nReceived TE message from IP:  ");
        for (int i=0; i<4; i++)
          printf("%d.", ((unsigned char*)&ip->saddr)[i]);

        // flag_recvd_time_exeeded = 1;
        // break;
        return 0;
      }
    }
